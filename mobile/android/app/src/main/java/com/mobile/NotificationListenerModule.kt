...
class NotificationListenerModule(reactContext: ReactApplicationContext) :
    ...
    /**
     * 로그인 후 호출 — 백그라운드 Worker/Service가 RLS를 통과할 수 있도록
     * 현재 access token도 함께 저장한다.
     */
    @ReactMethod
    fun setUserSession(userId: String, accessToken: String) {
        reactApplicationContext
            .getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
            .edit()
    ...
    /**
     * 로그인 전 수집된 알림 텍스트 배열을 반환한다.
     * 서버 처리 성공 후 removePendingNotifications()로 성공 항목만 제거한다.
     */
    @ReactMethod
    ...
    /**
     * 성공적으로 서버에 반영된 큐 항목만 제거한다.
     * JS 처리 실패 시 데이터 유실을 막기 위해 getAndClear 대신 이 메서드를 사용한다.
     */
    @ReactMethod
    fun removePendingNotifications(processedTexts: ReadableArray) {
        val prefs = reactApplicationContext
            .getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        val existing = JSONArray(prefs.getString(QUEUE_KEY, "[]") ?: "[]")
        val processed = mutableSetOf<String>()
        for (i in 0 until processedTexts.size()) {
            processedTexts.getString(i)?.let { processed.add(it) }
        }

        val remaining = JSONArray()
        for (i in 0 until existing.length()) {
            val text = existing.getString(i)
            if (text !in processed) remaining.put(text)
        }
