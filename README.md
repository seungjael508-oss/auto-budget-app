...
- **자동 수집** — Android NotificationListenerService로 카드사 앱 알림을 백그라운드에서 자동 포착
- **텍스트 파싱** — AI 기반 `parse-text` Edge Function으로 알림 문자 → 거래 데이터 변환
- **CSV 업로드** — 카드사 명세서 CSV 직접 업로드 지원
- **공유하기 수신** — 앱 외부에서 텍스트 공유 → 자동 파싱
...
```bash
# mobile/.env 생성 (mobile/.env.example 참고)
...
### NotificationListenerService ADB 테스트
...
- `.env`, `local.properties`는 git에 커밋하지 않음
